...
func TestAddRule(t *testing.T) {
	...
	t.Run("add first rule to nil registry", func(t *testing.T) {
		...
		result, err := AddRule(t.Context(), nil, rule, nil, nil, new(EventMock), new(EventMock))
	...
	t.Run("add second rule to existing registry", func(t *testing.T) {
		...
		registry, _ := AddRule(t.Context(), nil, rule1, nil, nil, new(EventMock), new(EventMock))
		...
		result, err := AddRule(t.Context(), registry, rule2, nil, nil, new(EventMock), new(EventMock))
	...
	t.Run("add third rule to registry with two rules", func(t *testing.T) {
		...
		registry, _ = AddRule(t.Context(), registry, rule2, nil, nil, new(EventMock), new(EventMock))
		...
		result, err := AddRule(t.Context(), registry, rule3, nil, nil, new(EventMock), new(EventMock))
...
func TestAddRuleSameSourceChaining(t *testing.T) {
	...
	t.Run("two rules with different sources have no same-source links", func(t *testing.T) {
		...
		registry, _ := AddRule(t.Context(), nil, &MockRule{
			...
			Then: &MockAction{},
			To:   &MockDestination{},
		}, nil, nil, new(EventMock), new(EventMock))
